...
## Script 9 for sum score analysis
...
output_dir <- "./sum_score_analyses/ADE_model_Results_SA/corr"
...
for (scale_i in scales){
  ...
  for (rater in raters) {
    ...
    for (rated_age in ages) {
      ...
      keepMZ <- complete.cases(mz_data[ , cols])
      keepDZ <- complete.cases(dz_data[ , cols])
      
      mz_data <- mz_data[keepMZ, , drop=FALSE]
      dz_data <- dz_data[keepDZ, , drop=FALSE]
      ...
      mz1_coef <- summary(polr(as.factor(oph1) ~ cohort + sex1 + parent_Age, data = mz_data, Hess = TRUE))$coeff
      mz2_coef <- summary(polr(as.factor(oph2) ~ cohort + sex2 + parent_Age, data = mz_data, Hess = TRUE))$coeff
      
      dz1_coef <- summary(polr(as.factor(oph1) ~ cohort + sex1 + parent_Age, data = dz_data, Hess = TRUE))$coeff
      dz2_coef <- summary(polr(as.factor(oph2) ~ cohort + sex2 + parent_Age, data = dz_data, Hess = TRUE))$coeff
      ...
      Bage <- mxMatrix( type="Full", nrow=1, ncol=1, free=TRUE,
                        values=svb1_Age, label="bage", name="Bage" )
      ...
      Coh1 <- mxMatrix(type="Full", nrow=1, ncol=1, free=FALSE, 
                       labels=c('data.cohort'),name="coh1")
      
      #
      ExpMean <- mxAlgebra(expression = B0 + coh*Bcoh + sex*Bsex + AGE*Bage, name='expMean')
      ...
      pars      <- list(B0_, Bcoh, Bsex, Bage, ThreshFixed, pathA, pathC, pathD, pathE,
                        modA, modD, modC, modE)
      
      modelMZ   <- mxModel( pars, covA, covC, covD, covE, covP, ExpMean, Sex, Cohort, Age, Coh1, covMZ, expCovMZ, ExpMean, dataMZ, expMZ, funML, name="MZ" )
      modelDZ   <- mxModel( pars, covA, covC, covD, covE, covP, ExpMean, Sex, Cohort, Age, Coh1, covDZ, expCovDZ, ExpMean, dataDZ, expDZ, funML, name="DZ" )
      multi     <- mxFitFunctionMultigroup( c("MZ","DZ") )
      
      # ACDE model
      # Create Confidence Interval Objects
      ciACDE     <- mxCI(c("mA","mD","mE"), interval = 1 - sig_thresh)
      ...
      otestAD_out    <- mxTryHard( otestAD, intervals=T)
      ...
      model_info <- data.frame("scale" = c("", scale_name),
                               "rater" = c("", rater_name),
                               "age" = c("", rated_age)
      )
      
      test_res <- rbind(test_AD, test_E, test_OM)
      test_res <- cbind(test_res, model_info)
      
      test_res <- test_res %>%
        mutate(is_sig = ifelse(is.na(p), "", p < sig_thresh))
      ...
      models <- list(ofitACDEmod2, otestAD_out, otestE_out, om_test_out)
      ...
      for (each_model in models) {
        ...
        if (model_name == ofitACDEmod2@name) {
          is_sig <- NA # The base model does not have a p-value against itself
        } else {
          model_p_val <- mxCompare(ofitACDEmod2, each_model)$p[2]
          is_sig <- model_p_val < sig_thresh
        }
        ...
        mz_desc <- mz_data %>%
          group_by(cohort) %>%
          summarise(
            MZ_poly_cor = purrr::possibly(function(x, y) polychor(x, y, ML=TRUE, std.err=TRUE)$rho, otherwise = NA)(oph1, oph2), # <-- FIXED
            n_MZ = n(),
            .groups = 'drop'
          )
        ...
        dz_desc <- dz_data %>%
          group_by(cohort) %>%
          summarise(
            Dz_poly_cor = purrr::possibly(function(x, y) polychor(x, y, ML=TRUE, std.err=TRUE)$rho, otherwise = NA)(oph1, oph2), # <-- FIXED
            n_DZ = n(),
            .groups = 'drop'
          )
        ...
        res_df <- res_df %>%
          ...
          dplyr::select(
            Scale, Rater, Age, Model, is_sig, Birth.cohort, cohort,
            n_MZ, n_DZ, MZ_poly_cor, Dz_poly_cor,
            B_mA, B_mD, B_mE, var_phen, varA, varD, varE,
            std_varA, std_varD, std_varE
          )
      ...
      # Print progress
      cat("Processed:", scale_i, rater_name, rated_age, "\n")
...
write.xlsx(results_df, file = paste0(output_dir, "/ADE_parameters_corr_SA.xlsx"))
write.xlsx(trend_test, file = paste0(output_dir, "/ADE_ModelResults_corr_SA.xlsx"))

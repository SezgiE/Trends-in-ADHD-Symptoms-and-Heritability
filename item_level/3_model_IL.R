...
for (file in file_list){
  ...
  for (rater in raters){
    for (rated_age in ages){
      ...
      # Add orthogonal polynomials for birth cohort (uncorrelated linear & quadratic)
      orth_poly <- poly(data_wide$cohort, 2)
      data_wide$cohort_lin <- orth_poly[, 1]
      data_wide$cohort_quad <- orth_poly[, 2]
      
      print(round(cor(data_wide$cohort_lin, data_wide$cohort_quad),2))
      print(round(cor(data_wide$cohort_lin, data_wide$cohort),2))
      
      # Run multinomial regressions separately for Twin 1 and Twin 2
      mult_t1 <- multinom(as.factor(ph1) ~ cohort_lin + cohort_quad + sex1, 
                          data = data_wide, trace = FALSE)
      mult_t2 <- multinom(as.factor(ph2) ~ cohort_lin + cohort_quad + sex2, 
                          data = data_wide, trace = FALSE)
...
# Save Multinomial Regression
file_path_mult <- file.path(output_dir, "multinom_results.xlsx")
write.xlsx(multinom_results_df, file = file_path_mult, rowNames = FALSE)


# ---  summary of quadratic significance ---
...
cat("\n### Quadratic Term Significance Summary ###\n")
...
cohort_comparison <- multinom_results_df %>%
  ...
  dplyr::select(q_name, rater, age, twin, response_level, term, p_value) %>%
